        ...
        <div className=" min-w-40 flex flex-col justify-center   items-center ">
            ...
            {type === 'signup' ? <LabelledInput key={'name'}  label={"Name"} placeholder={"Lalith"} onChange={debounce((e:ChangeEvent<HTMLInputElement>)=>{
                setPostInputs((state)=>{
                    ...
                    <button type="button" onClick={sendRequest} className=" min-w-80 mt-8 text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">{type==="signup"? "Sign Up" : "Sign In" }</button>
...
function LabelledInput ({label, onChange, placeholder, type="text"}:LabelledInputType){
    return <div className="min-w-80 pt-6">
            ...
            <input  onChange={onChange} type={type} id={label} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder={placeholder} required />
    </div>
}