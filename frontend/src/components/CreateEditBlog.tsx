...
export const CreateEditBlog = () => {
    ...
    const publishHandler = async () =>{
       const response = await axios.post(`${BACKEND_URL}/api/v1/blog`,{...input },{
            headers:{
    ...
    return <div>
        ...
        <div className="flex justify-center  pt-8">
            <div className=" max-w-screen-lg w-full" >
                ...
                <TextEditor onChange={(text: string) => {
                    console.log(text)
                    setInput((state) => {
                        return { ...state, content: text }
                    })
                }} />
                <button onClick={publishHandler} className=" w-fit px-5 py-2.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg focus:ring-4 focus:ring-blue-200 dark:focus:ring-blue-900 hover:bg-blue-800">
                    Publish
                </button>
            </div>
